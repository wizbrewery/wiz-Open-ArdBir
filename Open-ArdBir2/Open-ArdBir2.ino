...
#define I2Ctempaddress 8 //added by wizbrewery for data transfer

//libraries
#include <EEPROM.h>
#include <LiquidCrystal_I2C.h> 
#include <OneWire.h>
#include <DallasTemperature.h> //Added by wizbrewery for dsb11 temp probe
#include <PID_v1.h>
#include <Wire.h>               //Added by OSN: Activate I2C Display
#include "I2C_Anything.h"       // added by wizbrewery for dataq transfer
...
// SETTING PCB*****
  ...
  #include "Pcb_11s.h"
#elif PCBType == 5
  #include "Pcb_OSN.h"  // for OSN and wizbrewery modifications
#endif
...
float tOffset; //added by wizbrewery

extern TwoWire Wire1; //added by wizbrewery for using 2nd i2c on mega
...
float boilStageTemp;
float Temp_Now;
float Temp_Mon; //added by wizbrewery monitoring probe temperature
...
void pauseStage(){
  ...
  if (btn_Press(Button_start, 250)) {
    ...
    while (stage_pause) {
      ...
      delay(45);  // wizbrewery reduced this value from 75
...
void Temperature(){ // modified by wizbrewery reads two DS18B20 temerature probes
  sensors.requestTemperatures();
  Temp_Now = Get_Temperature(OneThermometer)+ ((r_set(7) - 50) / 10.0);
  Temp_Mon = Get_Temperature(TwoThermometer) + ((r_set(7) - 50) / 10.0);
  if (ScaleTemp==1){
    Temp_Now = Temp_Now * 1.8 + 32.0;
    Temp_Mon = Temp_Mon * 1.8 + 32.0;
  }
//Transmit both temp on serial1 line added by wizbrewery

 #if PID_FE == true
    if(millis()>serialTime)  {
       I2C_writeAnything (Temp_Now);
       I2C_writeAnything (Temp_Mon);
       serialTime+=500;
    }
...
float Get_Temperature(DeviceAddress deviceAddress) { // by wizbrewery reads the DS18B20 temerature probe with the address given by passed argument
...
void heat_on() {
  ...
  digitalWrite(Heat_LED, HIGH);    //Added by OSN, status LED
  ledHeatON();
}

void heat_off(boolean mheat) {
  digitalWrite (Heat, LOW);
  digitalWrite(Heat_LED, LOW);    //Added by OSN, status LED
  ledHeatStatus(mheat);
}
...
void pump_on() {
  ...
  digitalWrite(Pump_LED, HIGH);    //Added by OSN, status LED
  ledPumpON();
}

void pump_off(boolean mpump) {
  digitalWrite(Pump, LOW);
  digitalWrite(Pump_LED, LOW);    //Added by OSN, status LED
  ledPumpStatus(mpump);
}
    ...
    if (pumpRest){
      ...
      if ((x - 1) == 8 && tempBoilReached && Temp_Now >= boilStageTemp) {  //if temp reached during boil
        ...
        */
        ...
        Stage((x - 1), stageTemp, Temp_Now, Temp_Mon);
...
void add_malt () {
  ...
  if (wait_for_confirm(1, 2, 1) == false) {
    LCD_Default(Temp_Now, Temp_Mon); // modified by wizbrewery for temp monitoring var
    delay(50);
    mainMenu = 0;
    b_Enter  = false;
  }
...
void manual_mode () {
  ...
  while (manualLoop) {            // manual loop
    ...
    if (SpentLeft) {
      ...
      LeggiPulsante(Verso, Timer);
      Manuale(mset_temp, Temp_Now, Temp_Mon, boilStageTemp);
    }
...
void Cooling() {
  ...
  while (f_Cooling) {
    ...
    Stage(9, stageTemp, Temp_Now, Temp_Mon);
...
void Whirlpool () {
  ...
  if (f_Whirlpool){
    ...
    while (f_Whirlpool) {
      Temperature();
      if (mpump) Stage(10, stageTemp, Temp_Now, Temp_Mon);
      else       Stage(11, stageTemp, Temp_Now, Temp_Mon);
...
void setup() {
  ...
  // added by wizbrewery setting the resolution of temperature sensors and startup
  sensors.setResolution(OneThermometer, 10);
  sensors.setResolution(TwoThermometer, 10);
  sensors.begin(); // IC Default 9 bit. If you have troubles consider upping it 12. Ups the delay giving the IC more time to process the temperature measurement
  Serial.println("Dallas Temperature IC Control Library Demo");
 
  // added by wizbrewery Switch on the backlight  
  lcd.init();
  lcd.init();
  lcd.backlight();
  ...
  pinMode (Pump, OUTPUT);
  pinMode (Buzz, OUTPUT);
  pinMode (Heat_LED, OUTPUT);      //Added by OSN: Status LED for Heater
  pinMode (Pump_LED, OUTPUT);     //Added by OSN: Status LED for Pump
  pinMode (Pwr12V_LED, OUTPUT);     //Added by OSN: Status LED for 12Vdc Power
  pinMode (Pwr220V_LED, OUTPUT);    //Added by OSN: Status LED for 220Vac Power
  w_StartTime = millis();
  
  digitalWrite(Pwr12V_LED, HIGH);    //Added by OSN
  digitalWrite(Pwr220V_LED, HIGH);    //Added by OSN
  //w_StartTime = millis();
...
void loop() {
  ...
  default:
    ...
    Temperature();
    LCD_Default(Temp_Now, Temp_Mon);  //modified by wizbrewery to add monitoring temp var
