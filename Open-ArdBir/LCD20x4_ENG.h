...
void LCD_Default(float TempA, float TempB){ //modified by wizbrewery to support 2nd temp probe
  Intestazione();

  PrintTemp(6,1,TempA,2);
  PrintTemp(6,2,TempB,2);
...
void Manuale(float Set, float TempA,float TempB, float TempBoil){//modified by wizbrewery to support 2nd temp probe
  lcd.setCursor(12,1);
  if (Set<100)LCDSpace(1);
  lcd.print(Set);
 // lcd.setCursor(2,2);
  PrintTemp(1,1,TempA,2);
  PrintTemp(6,1,TempB,1);
...
void Stage(byte Stage, float Set, float TempA , float TempB){
  lcd.setCursor(0,0);
  lcd.print(F(" AUTO --> "));
  lcd.print(stageName[Stage]);

  //lcd.setCursor(1,1);
  PrintTemp(2,1,TempA,2);
  PrintTemp(2,2,TempB,2); //added by wizbrewery

  lcd.setCursor(12,1); //modified by wizbrewery
  LCDSpace(2); //added by wizbrewery
  if (Set<100)LCDSpace(1);
  lcd.print(Set);
  lcd.write(2);
