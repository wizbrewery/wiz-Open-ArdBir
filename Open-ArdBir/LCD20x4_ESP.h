char *PIDName[]   ={"Uso ", "Constante kP", "Constante kI", "Constante kD", "SampleTime  ", "Finestra  ms", "PWM         ", "Calibracion ", "Histeresis  "};
...
void LCD_Default(float TempA, float TempB){
  Intestazione();
...
void Stage(byte Stage, float Set, float Temp){
  ...
  lcd.setCursor(1, 3);
  if (Stage ==  0)               lcd.print(F("SU* *BAJ Pausa ---"));
  if (Stage  >  0 && Stage  < 8) lcd.print(F("SU* *BAJ Pausa STP"));
  if (Stage ==  8)               lcd.print(F("SU* *BAJ  ---  ---"));
...
void Credits(){
  ...
  display_lcd(3,2,"Traducciones:",750);
